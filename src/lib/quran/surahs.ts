/**
 * أسماء سور القرآن الكريم الـ114 للعرض في واجهة التسميع.
 *
 * الأرقام فقط هي ما يُرسَل إلى الخادم؛ مواضع الصفحات وأوزان النقاط
 * محفوظة في الخادم (`server/src/lib/surahs.ts`) وهو المرجع الوحيد لها،
...
/** اسم السورة برقمها، أو null إن كان الرقم خارج 1–114. */
export function surahName(number: number | null | undefined): string | null {
  if (number == null) return null;
  return BY_NUMBER.get(number)?.name ?? null;
}
